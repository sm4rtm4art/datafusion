...
# Parser vs Builder: Two Paths to the Same Plan:
...
:class: seealso
This dual-API architecture follows principles established in the broader data science ecosystem. For the theoretical foundation, see [Towards Scalable Dataframe Systems][dataframe-paper]. For the complete method reference, see [Transformations][transformations].
:::

With both APIs converging into the same [`LogicalPlan`], the next question is: what exactly lives inside a DataFrame? See [Anatomy of a DataFrame][anatomy-dataframe] for a deep dive into the [`LogicalPlan`] and [`SessionState`] that every DataFrame carries.
